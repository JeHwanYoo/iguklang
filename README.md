...
이 프로젝트는 이국랭의 JavaScript 구현체입니다.
...
| Keyword      | Action                                            |
|--------------|---------------------------------------------------|
| 이구(우+)욱      | 포인터가 가리키는 바이트를 `이구`와 `욱` 사이의 `우` 개수만큼 증가시킨다.      |
| 이구(구+)국      | 포인터가 가리키는 바이트를 `이구`와 `국` 사이의 `구` 개수만큼 감소시킨다.      |
| 고수?          | 포인터를 1 증가시킨다.                                     |
| 하-           | 포인터를 1 감소시킨다.                                     |
| 신            | 포인터가 가리키는 바이트의 값이 0이면, 대응되는 `킹갓 충무공 제너럴`로 이동한다.  |
| 킹갓 충무공 제너럴   | 포인터가 가리키는 바이트의 값이 0이 아니면, 대응되는 `신`으로 이동한다.        |
| 이국 왤캐 고수임?   | 포인터가 가리키는 바이트에 아스키 코드 값을 입력한다.                    |
...
입력(`이국 왤캐 고수임?`)은 stdin에서 1바이트씩 읽으며, 입력이 끝나면(EOF) 해당 셀에 `0`이 들어갑니다.

언어 특성상 사소한 실수로도 무한 루프에 빠지기 쉬우며, 디버깅이 매우 어렵습니다.
